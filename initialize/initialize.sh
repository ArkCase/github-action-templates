...
# Expected values:
#
#   ${ENV_FILE} == the environment file into which variables will be written for later sourcing
#   ${WORK_DIR} == the directory housing the container's checkout
#
...
cleanup()
{
	[ -v ARGS_TEMP ] && rm -rf "${ARGS_TEMP}" &>/dev/null
}
