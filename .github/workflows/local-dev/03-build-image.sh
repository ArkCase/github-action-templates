...
#
# Parameters: ECR_REGISTRY_PRIVATE, ECR_REGISTRY_PUBLIC, PARAM_PUBLISH_MAJOR, PARAM_PUBLISH_MINOR
#
...
#
# For local builds, allow the use of the local configurations. If those
# aren't set (as would be the case in the GitHub action), then we simply
# revert to generating our own files with our own authentication.
#
[ -n "${AWS_PROFILE:-}" ] || AWS_PROFILE="armedia-docker-build"

if [ -n "${AWS_CONFIG_FILE:-}" ] ; then
	[ -e "${AWS_CONFIG_FILE}" ] || fail "No AWS_CONFIG_FILE exists at [${AWS_CONFIG_FILE}]"
	[ -f "${AWS_CONFIG_FILE}" ] || fail "The AWS_CONFIG_FILE at [${AWS_CONFIG_FILE}] is not a regular file"
	[ -r "${AWS_CONFIG_FILE}" ] || fail "The AWS_CONFIG_FILE at [${AWS_CONFIG_FILE}] is not readable"
else
	if [ -f ~/.aws/config ] ; then
		AWS_CONFIG_FILE="$(readlink -f ~/.aws/config)"
	else
		AWS_CONFIG_FILE="${SECRETS_DIR}/aws_conf"
		(
...
BUILD_ARGS+=(--secret id=aws_conf,src="${AWS_CONFIG_FILE}")

if [ -n "${AWS_SHARED_CREDENTIALS_FILE:-}" ] ; then
	[ -e "${AWS_SHARED_CREDENTIALS_FILE}" ] || fail "No AWS_SHARED_CREDENTIALS_FILE exists at [${AWS_SHARED_CREDENTIALS_FILE}]"
	[ -f "${AWS_SHARED_CREDENTIALS_FILE}" ] || fail "The AWS_SHARED_CREDENTIALS_FILE at [${AWS_SHARED_CREDENTIALS_FILE}] is not a regular file"
	[ -r "${AWS_SHARED_CREDENTIALS_FILE}" ] || fail "The AWS_SHARED_CREDENTIALS_FILE at [${AWS_SHARED_CREDENTIALS_FILE}] is not readable"
else
	if [ -f ~/.aws/credentials ] ; then
		AWS_SHARED_CREDENTIALS_FILE="$(readlink -f ~/.aws/credentials)"
	else
		AWS_SHARED_CREDENTIALS_FILE="${SECRETS_DIR}/aws_auth"
		(
			echo "[${AWS_PROFILE}]"
			echo "aws_access_key_id=${ECR_AWS_ACCESS_KEY}"
			echo "aws_secret_access_key=${ECR_AWS_SECRET_ACCESS_KEY}"
		) |& sort &> "${AWS_SHARED_CREDENTIALS_FILE}"
	fi
fi
BUILD_ARGS+=(--secret id=aws_auth,src="${AWS_SHARED_CREDENTIALS_FILE}")
